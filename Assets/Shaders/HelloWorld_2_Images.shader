Shader "Unlit/HelloWorld_2_Images"
{
	Properties
	{
		...
		SubShader
	{
		...
		Pass
		{
			...
			const int nFrames = 2;				// Five frame support only
			...
			{
				return vec2[nFrames](vec2(0.0), vec2(0.0));
			}
			...
			{
				return vec2[nFrames](vec2(1.77489177 / VideoRes, 1.0), vec2(2.628205128205128 / VideoRes, 1.0));
			}
			...
			void main()
			{
				...
				float fInit = Blending(timeFrames[1].x, timeFrames[1].y, -0.5);
				//1
				frames[0].x += fInit;
				...
				float fFirst = Blending(timeFrames[0].x, timeFrames[0].y, 0.5);
				//2
				...
				offset += Blending(timeFrames[1].x, timeFrames[1].y, -offset) + 
								Blending(timeFrames[0].x, timeFrames[0].y, -0.25);
