Shader "Unlit/HelloWorld_4_Images"
{
	Properties
	{
		...
		SubShader
	{
		...
		Pass
		{
			...
			const int nFrames = 4;				// Five frame support only
			const float TimeDuration = 18.0;
			...
			{
				vec2[nFrames] frames = vec2[nFrames](vec2(0.0), vec2(0.0), vec2(0.0), vec2(0.0));
			...
			vec2[nFrames] GetFrameTimes()
			{
				...
				vec2 timeFrames[nFrames] = vec2[nFrames](vec2(0.0), vec2(0.0), vec2(0.0), vec2(0.0));
				for (int i = 0; i < nFrames; i++)
			...
			{
				return vec2[nFrames](vec2(1.77489177 / VideoRes, 1.0), vec2(2.628205128205128 / VideoRes, 1.0), vec2(1.77777778 / VideoRes, 1.0), vec2(0.9259259259259259 / VideoRes, 1.0));
			}
			...
			void main()
			{
				...
				float fInit = Blending(timeFrames[1].x, timeFrames[1].y, -0.25) + 
								Blending(timeFrames[2].x, timeFrames[2].y, -0.50) + 
								Blending(timeFrames[3].x, timeFrames[3].y, -0.75);
				//1
				frames[0].x += fInit;
				...
				offset += Blending(timeFrames[0].x, timeFrames[0].y, -offset) + 
							Blending(timeFrames[1].x, timeFrames[1].y, 0.25) + 
							Blending(timeFrames[2].x, timeFrames[2].y, 0.5) + 
							Blending(timeFrames[3].x, timeFrames[3].y, 0.75);
				finalColor += TextureColor(_MainTex, UVs[0]*Blending(timeFrames[0], 1.0, fScale), vec2(offset, 0.0), frames[0]);
				
				float fFirst = Blending(timeFrames[0].x, timeFrames[0].y, 0.75);
				//2
				...
				offset += Blending(timeFrames[1].x, timeFrames[1].y, -offset) + 
								Blending(timeFrames[0].x, timeFrames[0].y, -0.375);
				...
				frames[2].x += Blending(timeFrames[2].x, timeFrames[2].y, -frames[2].x) + 
								Blending(timeFrames[1].x, timeFrames[1].y, 0.5) + 
								fFirst + 
								Blending(timeFrames[3].x, timeFrames[3].y, -0.75);
				...
				offset += Blending(timeFrames[2].x, timeFrames[2].y, -offset) + 
							Blending(timeFrames[1].x, timeFrames[1].y, -0.375);
				...
				frames[3].x += Blending(timeFrames[3].x, timeFrames[3].y, -frames[3].x) + 
								Blending(timeFrames[2].x, timeFrames[2].y, 0.25) + 
								Blending(timeFrames[1].x, timeFrames[1].y, 0.5) + 
								fFirst;
				...
				offset += Blending(timeFrames[3].x, timeFrames[3].y, -offset) + 
							Blending(timeFrames[2].x, timeFrames[2].y, -0.375);
				finalColor += TextureColor(_FourthTex, UVs[3] * Blending(timeFrames[3], 1.0, fScale), vec2(offset, 0.0), frames[3]);
