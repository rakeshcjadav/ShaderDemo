Shader "Unlit/HelloWorld_3_Images"
{
	Properties
	{
		...
		SubShader
	{
		...
		Pass
		{
			...
			const int nFrames = 3;				// Five frame support only
			...
			{
				vec2[nFrames] frames = vec2[nFrames](vec2(0.0), vec2(0.0), vec2(0.0));
			...
			vec2[nFrames] GetFrameTimes()
			{
				...
				vec2 timeFrames[nFrames] = vec2[nFrames](vec2(0.0), vec2(0.0), vec2(0.0));
				for (int i = 0; i < nFrames; i++)
			...
			{
				return vec2[nFrames](vec2(1.77489177 / VideoRes, 1.0), vec2(2.628205128205128 / VideoRes, 1.0), vec2(1.77777778 / VideoRes, 1.0));
			}
			...
			void main()
			{
				...
				float fInit = Blending(timeFrames[1].x, timeFrames[1].y, -0.33) + 
								Blending(timeFrames[2].x, timeFrames[2].y, -0.66);
				//1
				frames[0].x += fInit;
				...
				offset += Blending(timeFrames[0].x, timeFrames[0].y, -offset) + 
							Blending(timeFrames[1].x, timeFrames[1].y, 0.33) + 
							Blending(timeFrames[2].x, timeFrames[2].y, 0.66);
				finalColor += TextureColor(_MainTex, UVs[0]*Blending(timeFrames[0], 1.0, fScale), vec2(offset, 0.0), frames[0]);
				
				float fFirst = Blending(timeFrames[0].x, timeFrames[0].y, 0.66);
				//2
				...
				offset += Blending(timeFrames[1].x, timeFrames[1].y, -offset) + 
								Blending(timeFrames[0].x, timeFrames[0].y, -0.33);
